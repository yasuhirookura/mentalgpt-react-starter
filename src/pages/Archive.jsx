...
export default function Archive() {
  ...
  const fetchPage = async (isFirst = false) => {
    ...
    try {
      // ✅ 最新データの保存先： users/{uid}/posts
      const col = collection(db, "users", user.uid, "posts");

      // status は既存レコードに無い可能性があるので、
      // 取得後にJS側で tab をフィルタ（無ければ archived 扱い）
      const base = query(col, orderBy("createdAt", "desc"), limit(PAGE_SIZE));
      ...
      const docs = snap.docs
        .map((d) => {
          ...
          // いろんなスキーマに耐えるように、本文候補を拾う
          //（必要ならここだけ好みに合わせて整えてOK）
          const userText = raw.userMessage ?? raw.content ?? raw.text ?? "";
          const aiText = raw.gptResponse ?? raw.aiResponse ?? raw.response ?? "";

          // アーカイブ表示の本文（ユーザー文＋AI文をまとめて見せる）
          // ※「ユーザー文だけ表示したい」なら content: userText にしてください
          const merged =
            aiText && userText
              ? あなた：${userText}\n\nMentalGPT：${aiText}
              : (userText || aiText || "");
          ...
          return {
            ...
            status: raw.status ?? "archived", // 無いものは archived 扱い
  ...
  const actions = useMemo(
    () => ({
      async remove(id) {
        if (!user) return;
        // ✅ users/{uid}/posts/{id} を更新
        const ref = doc(db, "users", user.uid, "posts", id);
        const grace = Timestamp.fromDate(
